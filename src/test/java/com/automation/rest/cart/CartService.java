...
public class CartService {
    ...
    private static final String OCIP_APIM_SUBSCRIPTION_KEY_oss = "8f7a4f1cbde04affb9c872fc2405e8b1";
    ...
    private static final String GET_ORDER_DETAILS = "/abs/acceptancepub/oss/ossservice/api/v1/customer/%s/channel/mobile";
    ...
    public <T> HttpResponse getOrderDetails(String userName, String password, String customerId, String storeId, String serviceType) {
        GetCartItemsResponse getCartItemsResponse = null;
        String url = HOST + String.format(GET_ORDER_DETAILS, customerId);
        HttpRequest request = new HttpRequest();
        request.setUrl(url);
        AccessToken accessToken = new AccessToken(userName, password);
        String accessTokenString = accessToken.getAccessToken();
        ...
        Map<String, String> headers = RestApiConfig.getJsonHeaders(accessTokenString);
        headers.put("Content-Type", "application/json");
        headers.put("Ocp-Apim-Subscription-Key", OCIP_APIM_SUBSCRIPTION_KEY_oss);
        request.setUrl(url);
        request.setHeaders(headers);

        Map<String, Object> queryParams = new HashMap<>();
        queryParams.put("storeId", "1502");
        if (!serviceType.isEmpty() && serviceType != null) {
            queryParams.put("serviceType", serviceType);
        }
        request.setParams(queryParams);
        ...
        try {
            ...
            throw new RuntimeException("Get order Details failed, exception message: " + e.getMessage());
        }
        return response;
    }
